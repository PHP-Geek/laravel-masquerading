var dtable = $('#sessionDatatable').DataTable({
    ...
    ajax: {
        "url": base_url + "/admin/session/datatable",
        'method': 'POST',
        'data': function (d) {
...
//        {data: 'P', name: 'projectSessionCount', searchable: true, orderable: false, title: "No of sessions", visible: true},
        ...
        {data: 'sessionListId', name: 'sessionListId', searchable: true, orderable: false, title: "Participants/Kit Lists", visible: true, render: function (data, full, row) {
                if (data != null) {
                    if (row.sessionListId != 0) {
                        return ' <a type="button" class="btn yellow btn-outline btn-xs" title="Participants" href="javascript:;" onclick="participantList(' + row.id + ')">' + row.listName + '</a>';
                    } else if (row.kitParticipantId != '0') {
                        return ' <a type="button" class="btn yellow btn-outline btn-xs" title="Participants" href="javascript:;" onclick="KitList(' + row.id + ')">' + row.kitParticipantName + '</a>';
...
function participantList(id) {
    window.location.href = base_url + '/admin/session/list/participants/' + id;
}
//go to kit list
function KitList(id) {
    window.location.href = base_url + '/admin/session/list/kit-list/' + id;
}
//filter the data 
...
function editSession(id) {
    window.location.href = base_url + '/admin/session/edit/' + id;
}
function addSession() {
    window.location.href = base_url + '/admin/session/create';
}
function copySession(id) {
    window.location.href = base_url + '/admin/session/copy/' + id + '/1';
}
