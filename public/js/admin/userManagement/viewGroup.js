var dtable = $('#groupDatatable').DataTable({
    ...
    ajax: {
        "url": base_url + "/admin/group-datatable",
        'method': 'POST',
        'data': function (d) {
    ...
    columns: [
        ...
        {data: null, searchable: false, orderable: false, title: 'Action', visbile: true,
            render: function (data, full, row) {
                return '<a onclick="groupHierarchy(' + row.id + ')" href="javascript:;" class="btn green-jungle btn-outline btn-sm"><i class="fa fa-eye"> View Group Hierarchy </i></a> <a href="javascript:;"onclick="edit(' + row.id + ')" class="btn green btn-sm"><i class="fa fa-edit"></i></a>';
            }
        }
    ]
});
...
function edit(id) {
    window.location.href = base_url + '/admin/group/edit/' + id;
}
function groupHierarchy(id) {
    window.location.href = base_url + '/admin/group/hierarchy/' + id;
}
...
function groupStatus(groupId) {
    swal(
        ...
        if (isConfirm) {
            ...
            $.post(base_url + "/admin/change-group-status", {_token: $("input[name=_token]").val(), id: groupId, groupStatus: status}, function (data) {
                if (data.code == '1') {
                    if (status == '1') {
...
function addGroup() {
    window.location.href = base_url + '/admin/group/add';
}