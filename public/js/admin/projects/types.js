var dtable = $('#projectTypeDatatable').DataTable({
    ...
    ajax: {
        "url": base_url + "/admin/project-type-datatable",
        'method': 'POST',
        'data': function (d) {
    ...
    columns: [
        ...
        {data: null, searchable: false, orderable: false, title: 'Action', visbile: true,
            render: function (data, full, row) {
                return '<a  href="' + base_url + '/admin/project-type/edit/' + row.id + '" class="btn green-jungle btn-outline btn-xs"><i class="fa fa-edit"></i></a>';
            }
        }
    ]
});
...
function changeProjectTypeStatus(projectTypeId, status) {
    ...
    $.post(base_url + "/admin/change-project-type-status", {_token: $("input[name=_token]").val(), id: projectTypeId, projectTypeStatus: status}, function (data) {
        if (data.code == '1') {
            dtable.draw();
        } else {
