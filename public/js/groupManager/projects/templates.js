var dtable = $('#projectTemplateDatatable').DataTable({
    ...
    ajax: {
        "url": base_url + "/group-manager/project/templates/datatable",
        'method': 'POST',
        'data': function (d) {
...
function editProjectTemplate(id) {
    window.location.href = base_url + '/group-manager/project/template/edit/' + id;
}

function addProjectTemplate() {
    window.location.href = base_url + '/group-manager/project/template/create';
}
function useProjectTemplate(id, type) {
    window.location.href = base_url + '/group-manager/project/template/use/' + id + '/' + type;
}
...
function deleteProjectTemplate(Id) {
    swal(
        ...
        if (isConfirm) {
            ...
            $.post(base_url + "/group-manager/project/delete-template/" + Id, {_token: $("input[name=_token]").val()}, function (data) {
                if (data.code == '1') {
                    dtable.draw();
                } else {
